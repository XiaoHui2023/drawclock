...
def test_div_r_uses_division_sign_with_variable_bottom_dot() -> None:
    div_r = importlib.import_module("drawio_lib.components.div_r")
    html = div_r.label_html()
    assert ">1</span>" not in html
    assert "%ratio%" in html
    mid = div_r.G.body_mid_y
    from drawio_lib.components.simple_shapes import DIV_SYMBOL_DOT_OFFSET
    from drawio_lib.components.div_r_component import DIV_R_DENOMINATOR_Y_OFFSET

    ratio_y = mid + DIV_R_DENOMINATOR_Y_OFFSET
    ratio_top = ratio_y / div_r.GRAPHIC_H * 100
    assert f'cy="{mid - DIV_SYMBOL_DOT_OFFSET:.1f}"' in html
    assert f"M 14.5 {mid:.1f} L 25.5 {mid:.1f}" in html
    assert f"top:{ratio_top}%" in html
    assert "font-size:7px" in html


def test_div_r_variable_sits_in_bottom_dot_region() -> None:
    div_r = importlib.import_module("drawio_lib.components.div_r")
    ...
    denominator_y = overlays[0][1]
    assert denominator_y == mid + DIV_R_DENOMINATOR_Y_OFFSET
    assert mid < denominator_y < bottom_y
    assert denominator_y - mid >= 4
...
def test_div_r_three_digit_ratio_remains_readable() -> None:
    ...
    mid = div_r.G.body_mid_y
    denominator_y = div_r._COMPONENT._center_labels()[0][1]
