...
- [x] `FB-ROOT-016`：复现并解决公共 `from→gate→mux` 与私人 `from→gate→div→mux` 异深度数组中的公共输出总线碎裂或外绕。
- [x] `FB-BEND-017`：复现并消除四个错列 `source` 直接进入同一 mux 时被完整坐标—路由反事实严格支配的额外折点。
...
- [ ] `FB-ROUTE-023`：2026-09-16 用户确认发布版仍复发；旧 PASS 已撤销，必须重新自然复现并修复公共根主干过早横穿后纵向贯穿多行的问题。
