...
- **正确性优先的性能升级**：任何生成提速都不得放宽端口、拓扑、碰撞、交叉、折点、确定性或完整指标门；同一历史反例必须在公开入口下从冻结红灯变为双跑全绿，并保持既有正常与相邻回归通过。
- **真实规模一分钟门**：按输入字节、组件数、连接数和结构风险分开管理真实用户集与压力集。7302-byte、63 组件、110 边的当前真实规模回归要求每次生成低于 60 秒；更大压力图单独记账，不冒充用户 SLA。
- **强制收据闭环**：正式 runner 必须记录每轮墙钟耗时，并在预算超限、产物缺失、非确定性、指标 exact-set 缺失、历史 witness 残留或任一质量退化时非零退出；不能等用户运行后才发现。

## 2026-09-15

- **发布闭环**：产品提交 `9ab3939a762f83295f7c449afea373203819c0be` 的 Release run `34945409769` 全步骤成功；公开 Linux 资产 SHA‑256 为 `9c99598fd3f424a4bd4367a83bcfa77cb298f93f5e2855bd796197affc5837a5`，34 项且无 license-like 文件，CI 与本机独立公开下载复核一致。
- **脱离主干的远端纵降**：公共根支路即使自身只有 H‑V‑H 的单个远端竖段，也要从同一逻辑输出端口的兄弟支路推断共享纵向主干；远端竖段跨越多个视觉行时，候选必须继续沿主干到目标附近或整体外缘再横接。完整可见盒、连接关系、端点、方向、环、异网重叠为硬门；只有交叉严格下降才接受，最多增加两个折点时还必须至少减少两个交叉点和三个交叉事件。
- **结构覆盖与发布门**：新增 6 轴确定性 pairwise 结构矩阵，计划与实际覆盖分别记账并要求 106/106；13 个用例全部经公开 CLI 和统一 29 项质量系统。runner、源码树、Oracle、质量注册表哈希写入收据，任何复发、非目标质量失败、证据矛盾或期望状态不符均非零。该门置于 GitHub Release 构建前，不能被历史回执替代。
- **Oracle 完整性**：路线碰撞统一使用器件图形、名称和注释的完整可见边界；逻辑边必须从公开输入显式绑定，禁止从 SVG 数字序号猜测。候选搜索按方向逐步深入，不得用固定前 N 个候选截断覆盖。
...
- **发行目录收敛**：默认归档以 `libraries/*.xml` 平铺器件库，用户文档集中到 `doc/`；根级文档和旧 `drawio-lib/` 由 exact-set 门拒绝。
- **错列 MUX 终态门与边界主干闭环**：撤销只凭输入含 `layout_column` 就声称完成错列攻击的旧口径。冻结旧版在目标 MUX 真实右移 524.4px 时自然生成中部横穿路线；当前版沿外缘主干进入目标。攻击第 23 轮又捕获相邻源网 split-rejoin，树化器现以几何点唯一父节点构造折点优先树，避免轴状态投影重新成环；最终候选完成 24/24、R1–R7、27 张公开 SVG × 28 指标和全量功能回归。

## 2026-09-11

- **注释颜色**：新增 `description_color`，接受常见 CSS 颜色写法并规范化为稳定 sRGB 十六进制；独立终态质量注册表追加颜色指标。
- **Python 下界**：源码最低版本由 3.10 降为经真实入口验证的 3.9；静态兼容扫描与 Python 3.9/3.10/当前版本结果比较进入发布前检查。
- **默认附件**：最新的精简要求取代 2026-08-25 默认附件内含离线源码、项目 Skills 和全部示例的约定。默认包只保留冻结程序、短文档、器件库、最小示例、颜色字段示例和必须许可；源码由仓库与 tag 提供。

## 2026-09-09

- **条件式首列优先**：废弃“全部无约束源头强制第一列”。完整根设施及全部出边回第一列若不增加交叉、跨线、异网重叠、节点/标签碰撞或折点，则必须回第一列；若会退化则允许靠近下级直连。纯线长变长不能阻止安全回列，纯线长缩短也不能授权后移。
- **原子根 cohort**：共享直接汇聚下级的根必须联合移动和验收，不能逐根贪心；源节点种类、名称、额外输出和链深都不改变该规则。
- **跨线桥完整性**：最终 arc SVG 的每个异网正交真交叉必须恰有一条参与边绘制桥；零桥、双桥、孤立桥或 junction 冒充桥均失败，判定坐标与最终 SVG 四位序列化精度一致。
- **统一质量升级**：全公开图完整指标注册表增至 24 项；本轮以 26×24 指标、定向 bridge mutants、同输入强制首列反例及连续七轮递归攻击共同验收。

## 2026-09-08

- **独立全图质量系统**：最终 SVG 由独立检查器提取节点、边、完整点序列、折点、线段方向、交叉伙伴、异网重叠、物理设施和源侧总线；质量 runner 对每个公开输入执行同一 21 项指标 exact-set，条件项必须给出适用或不适用证明，测试用例不能自选指标。
- **公共 from 总线语义**：零入度、多目标且共用同一输出端口的 `from` 必须保持一个物理设施和一条源侧纵向分发总线；普通 gate/source 根的远距显示副本优化继续保留，不能再用它拆散公共 `from`。
- **递归对抗轮次**：反馈修复后的默认连续 clean 要求由五轮提升为七轮；任一复发清零，从 R1 重启。轮次 exact-set、源码、runner、Oracle、语义合同与逐 case 结果均写入机器收据。
- **维护范围**：移除会产生 520 节点的 128-clock 织网示例及其生成入口；维护和发行 exact-set 只包含少于 512 节点的公开示例，不对 512 及以上节点作质量声明。
- **自动发布闭环**：项目既有“每次修改通过检查后自动发布”约定属于常驻授权，不要求用户逐轮重复“上传”；交付完成必须依次取得当前验收、提交、推送、远端 workflow、tag/附件与下载资产消费证据，不能停在本地通过或 push 成功。
- **干净 checkout 证据闭包**：发行门除校验收据哈希外，还用 Git 索引确认收据和其引用的全部修复证据均已跟踪；本地存在但被 ignore 的证据不能再授权提交或 Release。workflow 同步升级 `actions/checkout@v5`，消除 Node 20 弃用告警。

## 2026-09-07

- **复发重新闭环**：旧 016/017 精确复现真实但覆盖不足，用户再次报告后撤销完成口径；当前基线新增混合 kind/异深/不同后级/额外消费者与四源三列复杂红灯，修复不依赖名称或坐标。
- **递归攻击门**：修复后必须连续通过 5 类异构轮次；任一目标复发清零并从 R1 重启，运行错误不计 clean，receipt 绑定源码、manifest、runner 与独立 Oracle。首轮 R5 seed 018 实际捕获后置 owner 覆盖，证明旧局部绿灯不充分。
- **结构边界**：公共/私人规则数组从根输出的两路以上合格子集识别，额外消费者、混合器件与不同后级不取消总线；每个 mux 仍须有逐路私有对侧根，多个共享根交叉耦合的消费域允许按全局质量分设施。
- **维护规模**：继续排除 512、1024、2048、4096 高压力输入与专属门禁；维护与发行证明只覆盖当前 16/64/128 代表规模，CLI 本身不设置硬上限。
- **远端闭环**：commit `b648705` 的 Release run `34085120202` 中，干净 CI 五轮攻击、Ubuntu 16.04 PyInstaller/staticx、publish 与公开资产回下载 frozen/source smoke 均成功；016/017 关闭。
- **异深度公共复用数组**：严格场景为唯一公共 `from` 分别经过每路独立 `gate_i→mux_i`，私人 `from_i` 经过各自 `gate_i→div_i→mux_i`；每路 gate 与 mux 一一对应，只有公共 from 复用。共享输出端口应形成整洁纵向总线，不能用同深直连或单个公共 gate 扇出的样例代替。
- **错列 source 入 mux 折点**：新增四个 `source` 直接进入同一多输入 mux、source 位于不同可行列的场景；以端口、障碍、交叉、重叠和坐标—路线联合反事实判断多余折点。
- **通用解决口径**：联网对照分层布局、端口约束、边直化与正交折点最小化方法，先扩展特性空间和独立 Oracle，再修改公共布局所有者；禁止实例名、样例号和固定坐标特判。
- **举一反三覆盖**：把异深/等深、自然/显式错列、3/4/6/8 路、正反声明顺序、端口置换、单/双公共根域、规则/远距非规则结构编译为 success/fault/boundary 覆盖；质量指标使用最终设施数、纵向通道数、联合轴支配、排列不变性和异网重叠，不以固定 x 或固定六路代替。
- **闭环发行**：全量 467 passed、5 skipped；Windows 新鲜解压冻结/源码消费通过。远端 run `34076329108` 的反馈门、Ubuntu 16.04 PyInstaller/staticx、publish 与发布后回下载 frozen/source smoke 全绿；`v1.0.0^{}` 指向 `59fee21`，Linux 资产 SHA-256 为 `48C56C1...D5B27`，包内 25 个自动布局 JSON 且无 512+ 输入。

## 2026-09-05

- **公共 `from` 复用数组**：新增严格成功边界，一个公共 `from` 与每行一个私人 `from` 接入 `mux2` 输入 0/1，mux 直接连接 clock；最终 SVG 必须只有一个公共显示设施和一条纵向总线。旧 `source→mux3→cell→clock` 样例保留为不同边界，不再代替此场景。
- **覆盖声明约束**：测试除几何结果外还要断言用户指定的器件类型和直接连接关系；近似结构通过只能证明自身，不能扩大为精确场景已复现。
