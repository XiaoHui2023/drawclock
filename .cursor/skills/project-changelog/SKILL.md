...
- **交替公共根发布示例**：新增 `27-interleaved-common-root-pads.json`，六组公共零入度根与独立私有 `from` 共同接入 `pad3`，再继续 `cell→clock`。最终 SVG 直接门要求公共网络只有一个显示设施和一个首纵向主干；该形态在冻结旧版与当前版均为成功边界，不冒充 ROOT-015 的旧版红灯。
