...
- **逐边与逐源路由统计**：质量报告增加每条边和每个逻辑源的交叉点、交叉对事件、垂直跨度与夹行数；测试端按最终几何独立复算，禁止生产实现自证。
- **三行分散扇出预算**：按可见器件聚类估算实际行距；消费端相隔不超过三行时保持单源，刚超过三行即可形成安全的双锚点候选，每个逻辑源单轮最多两个渲染锚点。
- **性能约束**：交叉归因只累计事件与集合，不构造相交边的笛卡尔积；1024、2048、4096 节点压力门继续受 5、10、30 秒预算约束。
