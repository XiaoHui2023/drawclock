...
- 11:27 反作弊/分类/结构搜索聚焦门 13/13 通过（0.72s）；原始复发夹具的当前 CLI + 独立 Oracle + 29 指标为 1/1 通过（99.99s）。覆盖模型现新增分支级 `target_common_depth=same|direct|gate`，并强制两个高风险场景：多数公共分支经 gate/错列外部目标直连，以及多数直连/中部错列目标经 gate；对应高风险三因子交互也进入 exact-set。这修正旧生成器把公共链深度错误建模为整图单值的 coverage escape。
- 11:20 结构错列回归实际运行 3/4 通过，两个复杂公开 CLI 用例均 clean；唯一失败是反作弊测试文件漏导入 `inspect`，NameError 在任何断言之前发生，不计产品或 Oracle 结果。该组耗时 494.53s，暴露可见边界轨道的性能缺口：大量已被节点/文字阻断的水平轨道仍进入了二次路线交互评分。现补导入，并在候选评分前用完整 `vertex_visual_box` 对水平可见边做线性障碍预筛；预筛只删除不可行边，不做 top-N 截断。
- 11:10 生产候选已对结构 case-008 双跑确定，两份 SVG SHA-256 均为 `181EB869...BD3570`。全图交叉点 `417→400`、事件 `1160→1127`、异网重叠 `0→0`、折点 `254→254`；`public_gate_02` 分支现沿公共主干到 y=3135.006 才横向分支，局部交叉点 `16→13`、事件 `52→42`，独立 Oracle 为 023 witness=0。原始 136 节点 corpus 也从 9 个 witness 收敛为 0，当前全图 1417 事件/428 交叉点、无异网重叠。
- 准备同时迁移当前绿灯测试、搜索器复现分类和反作弊测试时，首个补丁因同一文件在一份 patch 中出现两个 Update 段而被 `apply_patch` 拒绝，未改任何文件。下一步合并为每文件单一 Update：当前 CLI 必须 clean；Oracle 源码结构门必须保留上/下 `visual_bounds` 边界；复现搜索只接受精确目标指标红灯，混入其他失败时拒绝。
- 11:01 新 Oracle 正例与两个负例 3/3 通过（85.67s）；随后用冻结 baseline 公开入口双跑重签自然复现，batch `20260916T025118Z-31bcf5e6`、2/2 均直接命中 `FB-ROUTE-023`，`missing_issues=[]`，`--phase reproduce` 通过。修复授权现在成立。生产 owner 仅做与独立 Oracle 不同实现的同类抽象升级：把所有终态器件+文字可见框的上/下扩张边界纳入水平轨道可见图，再沿用既有整图事务硬门选择；不匹配节点名、固定坐标或固定行。
- 10:53 障碍边界可见通道加入后，独立 Oracle 已对当前图命中 `FB-ROUTE-023`；结构 case-008 的 `public_gate_02` 见证为通道 y=3146.784、全图交叉点 `417→414`、事件 `1160→1150`、局部事件 `52→42`、折点 `4→4`。原始 142 节点 corpus 在当前排布下命中 9 个同类可见通道，不再假定固定 target 名称或固定坐标；测试改为机械要求“至少一个合法候选严格减少局部交叉事件且折点不增”。
- 10:49 机器红灯按预期失败：目标测试 1 failed，旧 Oracle 对当前图返回 `detected_issues=[]`。独立几何诊断对目标边的连续 y 可见通道扫描找到 286 个无器件/文字碰撞、无同网环且交叉事件严格减少的候选；最优通道为 y=3140.0085，局部交叉点 `16→13`、事件 `52→42`、异网重叠 `0→0`、折点 `4→4`。旧 Oracle 只从现有路点/行中选轨道，没有从扩张后的器件与文字可见障碍边界构建可见图。现仅修独立 Oracle：加入每个终态 `visual_bounds` 的上/下扩张边界候选，生产 `src/**` 仍冻结。
- 10:45 复现测试已先改为当前正确期望：同一公开 CLI 输入必须命中 `FB-ROUTE-023`，并对 `public_gate_02` 记录局部交叉事件至少 `52→42`、折点不增；统一质量系统必须仅以 `premature_interior_trunk_entry` 报红。该测试在旧 Oracle 上预期失败，用作先红后绿的机器门；生产 `src/**` 仍未改。
- 10:41 当前 `f222dcdf` 自然红灯已重新成立：历史合法结构输入通过当前公开 CLI 生成的 `svg-edge-0159` 从公共主干 `x=416.36` 在 `y=2630` 提前横移到 `x=1622.16`，再纵降至 `y=3092.0085` 才连接 `public_gate_02`；该边 4 折点、16 交叉点、52 交叉事件。当前 Oracle 对同图返回 023 witness=0，因而产品失败与 Oracle escape 同时成立；`FB-ROUTE-023` 精确推进为 `reproduced`，旧 `current_passes` 改为 false 并保留撤销原因。
- 10:35 旧覆盖搜索器在冻结发布源码上执行了 10 个公开 CLI 用例后主动中止；每例现有逐边 witness 均为空，不计 clean。审查发现其 reproduced 条件同时要求“023 witness 存在”且“全指标无任何失败”，但 023 witness 本身就是注册指标，因而真红灯会被矛盾条件吞掉。同时图像复核确认现有因子把公共链深度设为全局单值，没有生成“大部分经独立 gate、离群分支直达后排 mux”的非对称结构。下一步先写入该正常输入并由终态整网 Oracle 判定；新增夹具的首次补丁因本条尚未入账被实时 worklog 钩子拒绝，未改文件。
- 用户确认实际运行后问题仍存在，证明上一轮结构矩阵与统一指标对真实输入等价类同时漏检。`FB-ROUTE-023`、错列 MUX 目标、发布完成声明和旧 delivery-ready 凭证立即作废。
- 逃逸分类为 `oracle_escape + coverage_escape + ownership_escape + claim_escape`：旧攻击生成器与 Oracle 共享“单边候选 + 已知 H-V-H 形态”的抽象，未能独立覆盖完整同网共享树、等价序列化形态、多个后置 owner 与发布程序真实输入分布。
- 生产 `src/**` 冻结。下一步先从 `v1.0.0/f222dcdf` 正常公开入口做结构引导搜索与双跑自然复现；新 Oracle 必须先拒绝漏放发布制品、接受干净基线并击杀旧实现 mutant，才允许修改路由 owner。
- 已重新设置活动目标，并触发用户否定恢复、频发错误专项、成熟项目对标、Find Skills、独立 Oracle 和机器门禁强化流程。
...
- 用户再次指出：公共纵向总线没有先沿主干到整体外缘，而是在提前数行处横向进入分支区，再纵向贯穿多行去连接更靠右的外部节点，产生大量交叉。此前 `FB-ROUTE-023` 完成结论和对应发布授权已撤销；当前状态为 `reproduction_in_progress`，生产路由 owner 保持冻结。
- 逃逸分类为 `coverage_escape + oracle_escape + claim_escape`。旧 `24 seeds + R1-R7` 用运行次数代理覆盖，没有机器输出“维度取值、全部高风险 pair、已知 triples”的 required/covered/missing exact-set；因此即使每轮 clean，也可能全部落在同一种目标纵区、分支密度、链深、额外消费者或后置 owner 组合中。
- 新复现合同不再规定成功轮数。覆盖轴至少包括：目标横向错列幅度、目标纵向位置（上/中/下/整体外部）、公共分支密度与跨度、公共/私有链深、目标端口与 fan-in/out、根的额外消费者、声明/命名顺序、显式/自由列约束、障碍带、设施 alias、后置路由 owner。停止条件是所有取值、指定高风险二元交互和已知三元交互均有公开 CLI 最终 SVG 收据，且 `missing=[]`；覆盖缺口会继续驱动生成新 case，而不是增加无方向的轮次。
- 修复授权仍要求自然红灯：合法公开输入、冻结旧 producer、公开 CLI 双跑、原始 SVG 未改写、独立逐线事实直接观测“提前横段 + 后续长竖段 + 穿越多行/多交叉”。故障注入只校准 Oracle，不能代替复现。
- 第一条覆盖方向输入已加入复现语料：8 行公共 `from -> 独立 gate -> mux` 与私人 `from -> gate -> div -> mux`，普通 MUX 固定列 5，最下行目标 MUX 固定列 8；它只作为正常入口输入，尚未取得终态红灯，因此不解锁生产修改。
- 首个 8 行底部错列输入由当前公开 CLI 生成后为 0 交叉、14 折点；公共根始终沿 x=191.3 的单一纵干线到目标行再横接，属于干净反例，不计复现。旧 Oracle 只枚举整图顶部/底部外廊，无法直接观察“离开既有公共主干后在远端纵穿多行”；现仅扩展独立终态判定器，新增基于逐段方向、目标行穿越和整图反事实的 lateral-departure witness，生产路由仍未修改。
- 新增覆盖驱动搜索器：以 9 个特性轴生成值覆盖、全 pairwise 与 3 组已知三元交互的精确集合；贪心覆盖数组只用于压缩合法组合，收据保留 required/covered/missing。复现模式只在终态 SVG 命中目标 witness 时成功，穷尽覆盖仍未复现则非零；修复模式必须完整跑完且 `missing=[]`。这取代按 seed 数量计 clean 的旧退出代理。
- 第一版覆盖模型完整执行 19/19 case、284/284 coverage units，`missing=[]`，但目标 witness 始终为空并按设计以 `not_reproduced` 非零结束；所有 case 的 29 项统一质量也无失败。它只证明“重复阵列内部的错列 MUX”方向干净，不能反驳用户看到的“阵列之外的右下外部目标”。缺口是把目标 membership 固定成阵列成员，以及没有覆盖公共设施多输出、独立尾分支和 alias/后置设施分割交互；下一步扩展这些正交方向，不重复增加 seed。
- 第二版模型增加 `target_membership=array|external` 与目标公共设施 `single|extra` 两个轴：external 是阵列之外的独立错列 MUX 尾分支，extra 让其公共设施另有消费者，从而覆盖设施分割、alias 与后置 owner 交互。新增的轴进入全 pairwise，并为 membership × 纵区 × 多输出建立已知三元 exact-set。
- 第二版 18/18 case、392/392 units 仍未复现并非零结束，说明“显式 `layout_column` 造成的错列”已干净但诱因仍不完整。第三版新增 `misalignment_cause=explicit-column|depth`：depth 不写 MUX 列约束，而是让目标私人链比普通行多 2/4 级，使 MUX 被真实拓扑层级自然推到后排；该轴与错列幅度、公共链深建立三元交互，直接覆盖用户所述“级数不同而不在同列”。
- 第三版 19/19 case、454/454 units 仍未复现并非零结束；抽查终态发现自然权重已把 depth 目标 MUX 与同类 MUX 对齐，且简单局部数组交叉始终为 0。这暴露两个模型问题：输入请求 depth 不等于终态错列，且没有障碍织网就无法触发“大量交叉”。第四版新增 `obstacle_pattern=local|reverse|cyclic`，让合法私人终端跨行逆序/循环连接形成真实横向障碍带，并与目标 membership 和纵区做三元覆盖；后续还须把终态 MUX 坐标语义加入收据，不能再用配置代理实际错列。
- 第四版 18/18 case、559/559 units 仍未复现并非零结束；终态抽查显示单一 reverse/cyclic 映射会被分层布局整体重排行以消除全部交叉，因此它不构成真实障碍带。覆盖模型新增 `double-weave`：每个合法 mux3 同时接入 reverse 与 cyclic 两种私人终端排列，形成相互冲突、无法靠一次行重排同时消除的障碍织网；这是改变拓扑交互方向，不是增加无意义轮次。
- 第五版 20 个压缩 case 完成全覆盖仍未命中目标；双织网已自然产生 456 个交叉事件，说明交叉压力成立，但抽查的 external“bottom”目标实际被交叉最小化排到顶部，暴露又一次输入标签代理终态几何的问题。模型新增 `external_anchor=first|middle|last|opposed`，用外部 MUX 的两条私人输入锚定不同排列区段；下一步按最终 SVG 的目标 y 分位与 x 错列直接分类，而不是相信期望 band 名。
- 覆盖收据现从最终 SVG 重算目标 MUX 的中心、相对普通 MUX 的实际右移量和 y 分位；只有 `target_right_offset_px>0` 的终态真实错列 case 才覆盖单位，`target_band` 以实际 top/middle/bottom 替换请求标签。终态前提不成立的 case 保留诊断但不计覆盖，也不能以“无 symptom”计 clean。
- 定向锚点实验取得两个边界：终态 rank=1.0 的右下外部 MUX 路线正确沿公共 x=242.22 主干到底再横接；rank≈0.54 的错列外部 MUX 则稳定生成 `right/down/right/down/right`，在 x=814.86 处纵穿 1400.04px、7 个交叉。后者尚未被 Oracle 接受，因为直接在目标行横接会穿器件；新增 lower(3/4) 锚点寻找“目标在主分支群下缘但非绝对底端”的用户精确几何，避免拿中部目标冒充右下目标。
- lower 锚点自然生成了精确红灯候选：外部 MUX 终态 x=1103.37、y 分位 0.792；公共边在 y=2318.31 提前离开 x=232.22 主干，随后于 x=804.86 纵穿 2279.02px 到 y=4597.33，产生 11 个交叉。枚举最终 SVG 已存在的可见水平通道后，y=4131.12/4518.85 可保持 4 折点、总长不增、异网重叠为 0，并把该边交叉从 11 降到 3、整图事件从 66 降到 58。旧 Oracle 只试目标行或整图最外边界，分别撞器件或变差；现扩展为终态 visibility-channel 反事实，仍未修改生产路由。
- 同一合法输入经当前公开 CLI 第二次独立生成，两个 SVG SHA-256 均为 `9f473d9059594d65fc57ee55004e0bdfe7019fed2d92c4cfe01b5b2923a42158`，两个 Oracle witness 逐字段一致：远端纵段 2279.0236px、跨 9 行、局部交叉 11→4、整图事件 66→59、4 折不变。`FB-ROUTE-023` 已从 `fixed_verified` 重开为 `reproduced`；现在才解锁生产 owner，优先修复最终边界路由候选集而非节点名或固定坐标。
- 生产修复落在最终多行 root corridor owner：除整图 top/bottom 外廊外，按当前终态全部路线的合法 y 轴枚举“沿公共主干继续到更靠近目标的可见通道”候选；候选仍必须不增加节点/可见框/端点/方向/异网重叠/折点，并严格减少整图交叉。没有使用样例名、MUX kind、固定行数或固定坐标；下一步先在精确红灯上重生成验证，不以源码推断成功。
- 修复首次执行被 `KeyError('local')` 阻止：新增 local 候选后，mandatory-lane 只为 top/bottom 建表但仍用直接索引。改为显式空集合后，同一冻结红灯双跑 SHA-256 均为 `c24f82013b7bbe708df838b98daf8359771888445370fbc90e29071d26511f83`；整图交叉点 55→48、事件 66→59，折点 210、曼哈顿总长 122828.0058、异网重叠 0 均不增加，023 witness 1→0。相关 corridor 聚焦测试 8/8 通过。
- 覆盖器随后暴露自身性能缺陷：约 41 万个全笛卡尔候选先物化再贪心，运行数分钟仍未完成，已主动中止；这不是产品红灯。下一版改为约束感知候选构造，直接生成每个值、所有 pair 与指定高风险 triple 的可适用证明，并把本次逃逸的 14 轴高阶组合设为具名必测场景；完成条件继续是 exact-set `missing=[]`，不是候选数或轮次。
- 首版约束候选降到 277 个并在 3.1 秒内算完 842/842 units，但压缩套件仍需 212 个 case：原因是候选只改变一对/三元维度，跨单位复用能力不足。它的 exact-set 正确但执行成本不可接受；继续加入固定种子的均衡全维候选池供 set-cover 组合多个覆盖单元，具名逃逸场景仍固定为首例，随机种子不作为完成条件。
- 固定种子只用于生成候选池后，候选 4350、压缩执行集 34，5.2 秒闭合 842/842 units。随后把精确红灯升为全指标回归时，统一门正确拒绝 `port_order`：double-weave 为制造不可消除障碍，对多组 MUX 写入互相矛盾的私人根端口顺序，12 个 merge 出现端口次序倒置。023 修复本身没有制造该问题，但这个样本不能冒充最终全绿证明；保留其原始红灯事实，同时增加只锚定外部目标、不扰乱普通行端口顺序的合法方向，重新寻找隔离复现。
- `external-anchor-only` 保持 24 个普通 mux 的本地输入，只让外部 mux3 增加 lower 锚定输入；当前版和冻结 `c21e734` 均为 154 节点/154 边、29/29 指标通过、4 个交叉、023=0，故它是有效 clean control 而非复现。覆盖搜索器下一步允许显式选择冻结 producer，并且目标复现只接受 29 项全绿背景上的 023 witness；带其它质量红灯的压力图保留诊断但不得解锁完成。
- 冻结 producer 搜索第一次在 case-000 操作失败，未生成图：相对 `--producer-root` 与切换后的 cwd 重复解析。参数现于执行前规范化为绝对路径，操作错误同时输出 exit 与 stderr 尾部；此失败不计覆盖或产品复现。
- 第二次 case-000 stderr 明确为输入 JSON 不存在：输出根仍相对调用仓库，旧 producer cwd 无法解析。`--output` 也在建目录前绝对化，确保 producer checkout 与证据 checkout 可分离；样本内容和 Oracle 未变。
- 路径修正后冻结 `c21e734` 完成 40/40 case 和 exact-set `missing=[]`，没有任何 023 witness；这证明单外部锚点及压缩套件覆盖到的普通/反向/循环方向不足以隔离复现。下一正交方向是 `dual-common-weave`：第二共享根经每行 gate 同时进入本行与反向行 MUX，构造密集多总线障碍带；它不把单用私人根按互相矛盾端口接入，避免沿用 double-weave 的 port-order 假红。
- 双公共织网 16/24 行分别为 123/179 个逻辑节点、155/227 边、68/92 个交叉事件，冻结版均 29/29 全绿且 023=0；它们成为合法高密度 clean controls。新增 `double-weave-shared-roots` 方向：保持已知 023 诱因，同时让每个私人初始 from 有第二个消费者，覆盖用户此前指出的“源头双输出”交互，并使 port-order 的单用根适用前提不成立；仍由全 29 指标决定能否作为隔离红灯。
- 多输出根的远端 probe、MUX 直入与首列 stub 三种变体都会改变设施分区并使冻结版 023 消失；其中首列 stub 和 probe 版本 29/29 全绿。对原双织网仅按源行重排 MUX 私人端口，冻结版也从 66 事件/023 红灯变为 59 事件、29/29 全绿、023=0；故根因是跨行织网、固定端口语义、错列外部目标的高阶交互。发行覆盖剔除有内在 port-order 红灯的 reverse/cyclic/double-weave，保留其为诊断复现；clean 门新增所有 case 的完整指标零失败及最终几何 semantic-direction exact-set。
- 当前 clean 搜索第 2 例被语义门拒绝为 target geometry missing；终态 SVG 实有 `external_mux`，但收据筛选仍把 MUX kind 写死为 mux2/mux3，新增合法 mux4 压力方向后发生 Oracle 逃逸。识别改为 schema 同族 `kind.startswith("mux")`，错误文本加入目标名和完整 factors；该次没有覆盖收据，不计 clean。
- 修正后 clean 推进至 11 个结果，第 12 个 202-edge 多输出根图在 Oracle 内持续数分钟；已主动中止，未产生完整 coverage receipt。分析确认 lateral-departure 对每条候选通道都重新执行全图 O(E²) crossing 扫描，导致组合后接近 O(E³/⁴)。改为一次计算全图基线，并对被替换边增量删除旧事件/加入候选事件；distinct point、事件数、overlap 与局部计数判据保持完全相同。
- 增量 Oracle 在冻结红灯/修复图分别约 28.7/28.1 秒完成，023 witness 保持 1/0，旧图 issue 集仍为 `FB-PORT-006,FB-ROUTE-023`，证明性能改造未改判定。新 clean 34/34 执行结束但门拒绝：结构 units 842/842，缺 `array/top/right-offset`，另有 root-first 3 例与 avoidable-bends 1 例。后者 `mux_03→clock_03` 存在等折等长、事件 4→2、整图 71→69 的合法 x 通道；生产 owner 的局部 top-3 beam 截掉了整图最佳候选，现扩大为有界 12 路整图事务。
- beam 扩大后 case-022 仍原样红。逐候选复核发现正确 x=1108.65 来自可见节点框边界，而生产 channel 集只有现有路线 x 与相邻中点，因此它根本不在 shortlist，非 beam 排名问题。最终单边 owner 现与独立 Oracle 对齐，加入所有终态可见框 left/right 后再构造中点；完整节点碰撞与整图事务仍负责拒绝不可用边界。
- 可见框通道修复后 case-022 的同一输入通过 29/29。三个 root-first 红灯均为公共 from 的多个物理设施；反事实移动到“最左根列”后交叉/重叠/折点不变，只有线长增加，正是用户规定必须首列的适用域。生产恢复器误取全体器件最小 x，而 Oracle 取根设施最小 x，导致候选移到不存在的更左根列并被碰撞/设施门拒绝；现统一为零入度设施的最小 x。
- 统一 first-x 定义后 case-020 仍红；生产报告显示公共根后排设施被 `facility-minimality` 内部代理拒绝，而独立 root-first Oracle 已用同一 L1+facility 反事实证明候选不新增可合并对。恢复事务删除这个重复且口径漂移的实现代理，仍保留节点/边/可见框/方向/重叠/完整 hard-vector；最终统一 `mergeable_root_facility` 指标继续审计全部图，若真产生可合并设施则整图失败。
- 删除重复代理后 case-020 同一输入通过 29/29。语义缺口 `array/top/right-offset` 不是不可实现：8 行、top 阵列成员、显式 offset=4 的合法公开输入终态右移 122.02px、y-rank=0、29/29 全绿。它加入具名必测场景，覆盖工具必须执行而非依赖贪心偶然选中。
- 新全覆盖前三例运行成本显著上升，已中止而不拼接收据。此前 beam=12 是基于“正确候选被 top-3 排名截断”的错误假设；加入可见框边界后正确 x 的局部得分已是最优，故恢复 3 路有界 beam，保留真正修复的 channel 集，再从头生成单一源码血缘收据。
- 最新覆盖 32/32、842/842 units 和 6/6 最终语义方向闭合，023 无复发；唯一红灯为 16 行多输出根双织网的一处 `crossing_treatment`。渲染器已检测交叉，但横段在交叉前只有 1.36px，固定半径 4px 的 bridge 被静默跳过。圆弧生成改为在实际段端/上一 bridge 间裁剪可用区间，交叉必须严格落在区间内部，并按实际跨度缩放半径；小净空不再退化成没有任何跨线标识。
- 首个自适应 bridge 仍被质量门拒绝，同时一次 pytest 路径误写为不存在的 `tests/test_layout_preview.py`，0 tests 不计证据。独立 bridge Oracle 以弧端点中点作为桥中心；非对称裁剪使中心偏离真实交叉并产生 missing/orphan。实现改为以交叉坐标为中心，半径取默认值、前侧可用长度、后侧可用长度三者最小值，保持中心身份精确且不再静默跳过。
- 中心保持版在原 case-020 上通过 29/29，渲染聚焦测试 75/75。新增 near-bend 单测直接锁定交叉距段起点 1.25px 时输出中心仍为 1.25 的缩放圆弧，防止未来恢复“净空不足就不画桥”的旧行为。
- 最终 coverage receipt 增加 producer `src` 树、runner、独立 Oracle 与指标注册表 SHA-256，防止改代码/门禁后复用旧 clean；第六次仅完成 4 例的无血缘运行已中止并从头重跑。通用归档检查器对现有公开包命中 `doc/licenses/NotoSansCJK-OFL-1.1.txt`；包内未携带字体文件，只有源码内四个轮廓数据，二进制包无需附带该许可文件。bundle allowlist、项目归档检查与 Release 说明移除该附件；仓库源文件不在二进制发行包中，源代码许可义务不被伪装为删除。
- 对许可边界复核后撤销上一条“仅删许可即可”的判断：冻结程序实际会包含 4 个标注源自 Noto Sans CJK 的字形轮廓，仍可能触发 OFL 随附义务。现从源码和二进制彻底移除这些轮廓，`工作频率` 与其它表头统一用普通 SVG text 和系统字体回退；冻结 smoke 改为验证文本 exact value。这样最终二进制归档没有第三方字体数据，也无需用删许可掩盖许可义务。
- 带新血缘的最终覆盖运行在第 11 个 207 节点高密度组合持续占用 CPU，10 个结果后长期无新收据；同时发现此前 `current-clean-v6` 旧进程仍残留竞争资源，已按精确 PID 停止旧作业，再停止本轮未完成作业，禁止拼接半份覆盖结果。性能根因位于独立 lateral-departure Oracle：虽然全图基线交叉已做增量化，但它仍把整图每个折点 y 当作每条公共根边的反事实候选，密集路线使候选数与逐候选边比较相乘。恢复方案保持 32-case exact-set 不变，只把候选语义收敛到公共兄弟目标行、当前边及其目标直接邻域的真实 y 通道；先在冻结自然红灯与当前绿灯上证明 witness 1/0 和关键数值不变，再从头运行单一血缘覆盖。
- Oracle 通道收敛后，冻结红灯仍为 023 witness=1、`66→59` 事件、推荐 y=`4419.1811`，当前修复图仍为 witness=0；最重 207 节点单例约 124.79 秒结束，相比前次超过 10 分钟未完成显著收敛，且未删任何 case、因子或指标。随后复核测试 diff 发现新增高阶场景测试插入了既有函数中间，导致旧基线的 `detected_issues` 与全 29 指标断言被意外移交给新函数；这属于测试结构回退，需先恢复旧函数完整合同，再保留新场景自己的相同全指标门。
- 已恢复旧 023 当前基线测试的 issue 断言和完整 29 指标执行，新具名高阶场景保留同样的全指标合同。带优化 Oracle 的最终覆盖推进到 10 个结果后再次停止，因为代码审计确认 runner 对每个 SVG 先直接 `oracle.analyze()`，随后 `evaluate()` 内部又执行同一个完整 analyze，最重 case 被重复收费两次。另有一次误把无参数 `run_feedback_fix_verification.py` 当成支持 `--help` 的 CLI，触发真实全语料运行；已立即通过会话中断，未采用它产生的任何半成品收据。下一步让质量系统拥有单次分析并同时返回质量收据与几何报告，公开 CLI 仍只返回质量结果；随后用等价性测试证明单次接口与旧 `evaluate` 的 29 项状态一致，再从头运行覆盖。
- `svg_quality_system.evaluate_with_geometry()` 现由质量系统自己单次生成完整终态报告与 29 指标收据，旧 `evaluate()` 仍包装同一入口并保持返回结构不变；覆盖 runner 不再先行重复调用 Oracle，既没有接收外部自报报告，也没有削弱独立性。语法检查与 023 旧基线、154 节点具名高阶场景、coverage exact-set 三项聚焦测试为 3/3 PASS（81.60 秒）。下一步从空输出目录重启最终覆盖，收据必须绑定新 runner/Oracle/registry 哈希。
- 进程级复核纠正了上一条对误启动 fix runner 的判断：外层会话中断后，其 Python 子进程继续并完整生成 group `20260914T104230Z-c6a501e6`，19 个收据均为 `fixed_verified`，证据目录共 228 个文件；收据中的 `source_tree_sha256=ae5fc35e…08cc` 与当前 src 树一致，`oracle_sha256=fa85a891…9f3e` 也与当前文件一致。该组尚未因“脚本跑完”自动获得发布权，仍须把证据纳入 Git、更新问题状态并让 release gate 独立验证。另发现第二次中止的 coverage 子进程成为孤儿并与 v3 竞争，已按精确 PID 停止；当前只剩唯一 v3 runner。用户根 `case-generalization`、`agent-quality-workflow`、`proactive-skill-learning`、`clock-tree-layout` 四项第一次因错误寻找普通 skill-creator 路径而报 `Errno 2`，改用 `.system/skill-creator/scripts/quick_validate.py` 后四项均 valid；通用 release archive checker 自测 PASS。
- 最终单一血缘 coverage v3 完整 PASS：32/32 case、843/843 required/covered units、6/6 必需终态几何方向，`missing_coverage_units=[]`、`missing_semantic_directions=[]`、`quality_failure_cases=[]`、`first_reproduction=null`。实际观察 11 个方向，除 array/external × top/middle/bottom × right-offset 全集外还包含 5 个 aligned-or-left 干净对照。收据绑定 producer src `21dcfedf…9c2`、runner `9b108a94…a36`、Oracle `fa85a891…9f3e`、29 项注册表 `f9beadff…f761`；完成依据是 exact-set 与终态几何覆盖，不是 32 这个次数。
- 问题账本已追加本轮自然红灯/coverage 证据并把 023 提升为 `fixed_verified`，结构门 19/19 PASS。当前历史反馈回归 group `20260914T110916Z-5d38f1ac` 为 clean，公开 SVG 新鲜生成后 27/27 × 29 指标 PASS。首次 release gate 按预期 FAIL 260 项，全部是新 fix-evidence 尚未 Git 跟踪，证明本地自签 JSON 不能绕过干净 checkout 血缘；下一步只精确暂存本轮 228 个 fix evidence、19 个 fix receipt，以及新 recursive receipt/evidence，再重跑同一 release gate。早期 8 行错列 JSON 已被覆盖模型的具名 array/top 与 external 方向替代，删除该未引用孤立 fixture，避免语料出现无合同文件。
- 精确暂存本轮 573 个 evidence/receipt 路径后，release gate 19/19 PASS；随后完整 pytest 收集 623 项并在 944.38 秒后得出 622 PASS、1 FAIL。唯一失败 `test_multi_from_roots_keep_one_facility_and_shared_bus` 直接显示旧设施最小化回退：`from_1/from_2/from_3` 各出现 `6/7` 可避免副本。此前为了修复 root-first 假拒绝而完全删除生产内 `facility-minimality` 代理过度，满足新列恢复却损坏“远距才拆分、可合并副本不得保留”的旧要求；所有完成/发布结论再次撤销。下一步先把该测试配置经公开 CLI 输出最终 SVG，由独立全指标系统自然确认对应 root-facility witness；然后让首列移动事务采用与最终 Oracle 相同的合并支配口径，而不是恢复漂移代理或放宽测试。
- `FB-ROOT-015` 已从 closed 重开为 `reproduction_in_progress`。公开 example `16-multi-from-clusters.json` 经当前 CLI 两次生成相同 SHA-256 `C9544354…3E44`；测试侧独立 L1 层稳定指出 `from_1/from_2/from_3` 的 `6/7` 相邻设施可合并，但正式 29 指标仍 PASS、`mergeable_root_facility_witnesses=[]`，确认这是新修复引入的自然产品回退叠加 Oracle escape。现有正式 Oracle 只尝试把一个根的全部设施合为一个，遇到合法远距消费带便整体拒绝，遗漏“保留远距分区、只合并其中一对相邻设施”的局部支配。先扩展独立 Oracle 为逐相邻设施 pair 的完整路线、碰撞、全图交叉/重叠/折点和显示成本反事实；只有同一公开 SVG 变红后才重新修改生产 owner。
...
- 11:28 本轮自然复发证明旧候选空间遗漏器件可视外框净空边界，攻击空间又只记录整组 `common_depth`，无法表达“仅目标公共分支直连/多一级”；搜索器复现条件还曾把目标 witness 与空失败集相与，逻辑上不可满足。用户根与项目专题已同步三项通用合同：可见通道必须包含每个终态视觉障碍的扩张上/下边界、因素必须分离多数深度与目标 override、复现只允许目标指标成为唯一红灯。项目覆盖门同时移除固定 13 案/106 pair 作为成功条件，改为每次动态重算并实测 required exact-set；长矩阵仍在执行，未完成前不发布。
- 11:31 扩展后的覆盖方向已全部实际执行：34/34 case、977/977 coverage unit、missing=0，且目标 `premature_interior_trunk_entry` 未复发；但完整质量注册表在 case-013 报 `downstream_corridor_tail_bend`、case-016 报 `avoidable_bends`，因此 `--expect clean` 正确非零退出并撤销完成判断。case-013 可联合下移 `mux_23` 与下游 20.9991 px，在交叉 2→2 下使总折点 60→56；case-016 的 `svg-edge-0128` 有等折点候选使全局交叉点 41→39、事件 51→49。下一步先用冻结 HEAD 对同输入差分，区分本轮回归与新揭露的存量缺陷，再分别修复；不得只把两项从注册表排除。
- 11:38 冻结 `f222dcdf` 对相同 case-013/016 也分别失败，确认是新覆盖揭露的存量缺陷而非本轮可见边界补丁回归。首轮通用补丁移除 final-single-edge 的 `sorted(... )[:3]` 并加入 downstream→boundary→source-lead→single-edge 的单调闭包；语法与结构反作弊 12/12 通过。case-013 仍红，生产选择报告显示 downstream 候选因一个 `new-visible-edge-node-hit` 被拒，说明与独立 Oracle 的可见碰撞/冲突闭包语义仍不一致；case-016 尚待在精确修复后复验。下一步先让生产报告记录具体新增 `(edge, node)`，定位差异，禁止放宽碰撞门。
- 11:41 生产诊断已把阻断对象精确到 `e78:mux_23→clock_23` 与 `n42:external_mux`：候选把该线从 y=1306.0079 上移到 y=1285.0088，进入生产器件外框底边 1303.015。独立 Oracle 的 SVG 视觉框延伸到 1310.785，因而把原路线视为“已穿文字”、候选视为同一旧 hit，错误允许其进一步进入器件本体。结论是 case-013 的 downstream 红灯为 Oracle 分级逃逸，生产拒绝正确；修复应同时比较 graphic hit 与 visual hit，禁止“原有文字相交”掩盖新增器件本体相交，并加入该边界反例。
- 11:43 downstream Oracle 现分别维护 visual 与 graphic hit 集合，候选新增器件本体命中即失败，即使相同 `(edge,node)` 已存在文字外框命中；新边界反例与原正/负校准 3/3 PASS，case-013 同一 SVG 的完整 29 指标转绿。移除 final-single-edge 固定 top-3 后，case-016 新鲜公开 CLI 输出也由 29 指标全绿；其等折点路线成功减少交叉，不再依赖候选排名恰好落入前三。下一步重跑原 023 复发夹具与 34 案/977 单元全矩阵；任何新失败继续重启闭环。
- 11:47 原始结构复发夹具在最新生产/Oracle 上重新通过公开 CLI、023 独立 Oracle 与完整 29 指标（1 passed，97.34s），证明附带修复未把公共总线拉回旧路径。用户根和项目质量专题已新增“同一 edge/node 对的碰撞严重度必须按器件本体、文字、注释、净空分层”的通用合同。全矩阵已从空目录重启，当前执行中，旧 34 案失败收据不复用。
- 11:49 第二轮矩阵执行到 8 案时主动中止，不计通过或失败：自审发现新抓到的 case-013/016 仅由动态 covering array 偶然保留，未来因素变化可能消失。必须把“旧 visual hit 掩盖新增 graphic hit”和“合法单边通道排在旧 top-3 之后”升级为具名 required historical escape，再从新 runner 哈希和新 required exact-set 的空目录重跑；中止不是产品失败，但旧批次血缘作废。
- 12:14 两项逃逸已成为具名 required historical scenarios，模型/分类/反作弊聚焦门 14/14 PASS；第三轮从空目录执行完成并通过：34/34 case、979/979 coverage unit、6 个 required 终态语义方向全部覆盖（实测 11 个）、missing=0、quality_failure_cases=0、first_reproduction=null。每案均执行完整 29 指标。r2 父 shell 中断后曾遗留精确命令行为 `...coverage-20260916-r2` 的孤儿 Python，占用 CPU 但未进入 r3 血缘；已核对 PID 后单独终止，r3 不受影响。长矩阵性能成本仍需在最终报告如实给出，禁止用固定 top-N 回退。
- 12:42 首轮全项目 pytest 在 78% 前累计 4 个失败，后段单测因无限候选深化持续数分钟；该批次已中止且不计完整证据。`pytest -x` 定位第一项为冲突闭包正例退化：新增 graphic-hit 门在允许把被移动障碍对应的外部路线目标纳入 ownership closure 之前就直接拒绝，导致应合法扩张的 fixture 从 1 witness 变 0。正确修复是把新增 visual/graphic hits 合并进入同一“可扩张或 unresolved”判定：只有命中未移动障碍时拒绝，命中已移动障碍时仍扩张闭包。另需优化 final-single-edge 的完整事务次数，当前功能正确但性能不达标。
- 12:43 Oracle 冲突闭包已先把 `new_visual_hits | new_graphic_hits` 统一送入 ownership expansion，移除过早 graphic 拒绝；尚需补充闭包结束后的双层 subset 终态断言并重跑正例/严重度反例，当前为中间态，不计绿灯。
- 13:07 Oracle 闭包完成双层终态 subset 断言；可扩张冲突正例、同 pair 严重度升级负例和原 tail-bend 正例 3/3 PASS，case-013 完整 29 指标继续 PASS。几何测试拆分验证：67 个非重型用例 46.08s 全绿；15 个公开 CLI 复杂组合 985.42s 全绿，合计 82/82，无早先四项失败残留。长耗时来自真实公开入口与整图 Oracle，不降指标；下一步从零跑完整项目套件，随后才可进入发行门。
- 13:27 第二次完整套件在 78% 前仅出现 3 项失败，均已由 `tests/test_feedback_reproduction_gate.py` 独立归类：现有 fix receipts 源树/runner/Oracle 血缘陈旧，且 FB-ROUTE-023 仍保持 `reproduced`，因此 release-gate 正确拒绝；不是产品测试回归。该文件 21 PASS/3 expected-red。需在源码稳定、全量通过后重签全部 fix receipts 并把 023 推进 closed，不能现在伪造绿灯。性能审计还发现最终固定点闭包即使 downstream 零 move 也重复执行 boundary/source/single 全扫描；应在零 move 立即终止，并用与 endpoint signature 等价的 source/target clearance 预筛删除不可行单边通道，二者都不做 top-N 截断。
- 13:30 无损性能补丁已加入：single-edge 候选在 clone/全图评分前执行与 `_route_endpoint_signature` 相同的首末竖段净空判定；最终闭包在 downstream 零 move 时直接结束，不重复刚执行过的三个 route-only owner。自审发现 downstream 有 move、其余 owner 零 move 时仍必须再进一轮确认固定点，当前控制流尚需微调后再测试，此中间态不计性能或质量绿灯。
- 13:38 固定点控制流回读确认：只要 downstream 接受过移动，就完整执行 boundary/source-lead/single-edge 并无条件回到下一轮；仅当下一轮 downstream 为零移动时退出，因此没有把“其余 owner 零移动”误当作整体固定点。高密 case-030 的一次内部诊断正常完成、未遗留进程；此前公开生成 165.88s 的结果表明无损预筛只带来有限收益。暂不以固定 top-N 或未经证明的局部阈值换速度，性能成本进入最终报告，正确性仍由完整有限候选集和全图事务门保证。
- 14:00 最终候选的结构搜索与 SVG Oracle 聚焦套件完整退出：94 passed in 989.88s。该批次包含真实公开 CLI、可见文字/器件本体碰撞严重度、目标红灯分类、历史 top-N 逃逸和原始 023 用例；没有以单元桩替代制品验证。下一步在空目录重建 34 案 covering-array + 历史逃逸矩阵，旧 r3 收据因 producer/Oracle 哈希变化不再作为最终证据。
- 14:01 首次启动最终矩阵时沿用了另一结构 runner 的状态名 `clean_verified`，本 runner 的公开 CLI 仅接受 `clean|reproduced`，argparse 在生成任何 case 前以 usage error 退出；空 r4 目录没有被当作覆盖证据。按其真实接口改用 `--expect clean` 重新从零启动。
- 14:17 最终 r4 矩阵明确成功退出：34/34 case、required/covered=979/979、missing=0；6 个 required 终态语义方向实测覆盖 11 个、missing=0；quality_failure_cases=[]、first_reproduction=null。每案保存独立输入、SVG 和完整 29 指标结果。运行期间一次只读汇总错误选择超大数组字段导致输出截断，随后只列 schema 键和长度完成独立回读；截断命令不影响收据。下一步执行发布工作流中的结构复发搜索与递归攻击，不用 r4 替代其它门。
- 14:33 结构复发 runner 完整退出 `clean_verified`：planned/executed=13/13，计划与实测 pairwise 均为 106/106，reproductions、quality_failures、unexpected_quality_failures、evidence_inconsistencies 全空；runner/producer/Oracle/quality-registry 四类 SHA-256 已写入 receipt。历史 case-008 作为独立结构案通过完整 29 指标。下一步从 R1 开始运行保留语料的递归变形攻击。
- 14:52 递归攻击在 R6 自主重新命中真实复发并以 `quality_failure` 非零退出，R1–R5 的 clean streak 全部作废，未执行 R7：`boundary-offset-seed-018` 唯一失败指标为 `premature_interior_trunk_entry`。终态公共根边 `svg-edge-0120` 在共享 x=426.36 上未沿主干到可行出口，反而于 y=533.9934 提前横到 x=2112.46 后纵降并穿越两行；Oracle 给出的同折点候选改在 y=341.0014 横出，整图 crossing events 1303→1302、局部 11→10。该命中证明 r4/13 案虽覆盖既有因素，仍没有封住高交互 seed-018；下一步诊断生产 boundary owner 的候选生成/采用拒绝原因，修复后所有矩阵与 R1–R7 必须从零重跑。
- 15:02 将 Oracle 候选精确注入最终生产 `LayoutDocument` 后确认它通过全部生产硬门：逻辑环 0→0、节点/边节点碰撞 0→0、异网重叠 0→0、交叉点 370→369、事件 1303→1302、折点 252→252；仅长度增加约 300px。生产最终 boundary owner 的 86 次事务全部因 crossing/cycle 拒绝，说明该合法候选从未进入事务。轴值审计定位根因：生产只加入器件视觉框的 preferred-clearance 边界（目标框 top=349.0014，候选 y=331.0014），而合法通道位于同一视觉障碍与首选净空间的内部 y=341.0014；固定单一 expansion 不是完整可见图。通用修复加入两级障碍 expansion：18px 首选净空和由网格尺度导出的 8px stroke-safe 最小间距，仍由全图碰撞/环/交叉/折点事务决定是否采用，不按 seed、名称、坐标特判；公开回归参数加入 seed-018。尚未运行，不能计绿。
- 15:18 新增 seed-018 先由公开 CLI + 独立 Oracle + 29 指标单案通过（113.34s）；随后同一参数化族 0/3/13/18/22/23 加原始 case-008 共 7/7 通过（791.79s）。两级障碍 expansion 未引入文字碰撞、同网环、重叠或旧 023 回退。现在在新源码哈希上把递归攻击从 R1 完整重启；旧 R1–R5 clean 与 R6 failure 仅作失败证据，不计新 streak。
- 15:34 新源码递归批次 `20260916T071825Z-de421941` 已从 R1 完整重启并达到 R1–R7 连续 7/7 clean；决定性的 R6 共 49 案，包含原失败 `boundary-offset-seed-018`，不再命中 023 或其它 29 指标。状态为 `clean`、consecutive_clean_rounds=7。下一步把本轮“首选净空单一 expansion 逃逸、两级可见障碍边界、R6 命中必须升级为具名回归”的知识写入用户根与项目专题，然后因项目文档/测试/源码哈希最终固定，重建 34 案矩阵、结构搜索、全测试与发行证据。
- 15:37 用户根 `clock-tree-layout`、`case-generalization`、`proactive-skill-learning` 与项目算法专题已写入两级 obstacle expansion 和晚轮复发升级规则。首次 quick_validate 使用 Windows 默认 GBK 读取 UTF-8 `SKILL.md`，四项均抛 `UnicodeDecodeError`，该批不计验证通过；这是验证器入口编码问题，不影响文档内容。按既有跨平台合同设置 `PYTHONUTF8=1` 后必须对同四项重跑，若仍失败再修文档结构。
- 15:58 `PYTHONUTF8=1` 后四个用户根/项目 Skill 均由官方 quick_validate 判 valid。最终源码的 r5 覆盖矩阵从空目录完成：34/34 case、979/979 required/covered units、全 29 指标零失败，runner 明确输出 `clean coverage complete`。两级净空使高密搜索成本上升但没有恢复 fixed top-N；下一步重跑 13 案结构矩阵并重签其四类血缘哈希。
- 16:15 最终结构矩阵再次完整退出 `clean_verified`：planned/executed=13/13、计划与实测 pairwise 106/106，reproductions/quality_failures/unexpected_quality_failures/evidence_inconsistencies 全空；producer tree 新哈希为 `a102d026...bd4f`。现在运行全部历史 issue 的公开 CLI 双跑 fix verification；只有该批 failures=[] 后才允许把 023 从 reproduced 推进 fixed_verified 并运行全量 pytest/release gate。
- 16:33 全部历史 issue 的新验证组 `20260916T082756Z-b50fa7a1` 完成且 `failures=[]`；19 份 fix receipts 均绑定当前 source/library/runner/Oracle/semantics 哈希及每次证据文件。FB-ROUTE-023 已从 `reproduced` 推进 `fixed_verified`，`current_passes=true`，撤销旧 release verification，等待本次实际远端发布后才可 closed。下一步全量 pytest；若只有证据未跟踪类失败，按 fail-closed 规则精确暂存新验证组后重跑，不弱化 checker。
- 17:34 完整 pytest 首轮为 `635 passed, 2 failed, 5 skipped`（3413.11s）。两项均为账本 release gate，布局、Oracle、覆盖及其它功能测试全过；失败输出显示 023 状态仍为 reproduced。差异回读发现推进状态时用宽泛首个 `"status": "reproduced"` 锚点，误改了另一个 issue 的嵌套 `reopened_feedback.status`，而 023 本体未改；JSON 语法合法但语义 owner 错误，门禁正确失败。已精确恢复历史嵌套状态，并以 `id=FB-ROUTE-023` 相邻上下文把目标推进 fixed_verified。下一步先跑两项失败测试与结构门，再决定是否需要重跑完整套件；禁止把 635 个通过掩盖账本错误。
- 17:38 精确状态修复后，用户根 validator 与账本结构测试通过；项目 release gate 只剩预期的 260 项新 fix-evidence 未 Git 跟踪失败（2 passed/1 failed）。这证明状态 owner 已修正，剩余是 clean-checkout 证据闭包，不是功能回归。下一步仅加入验证组 `20260916T082756Z-b50fa7a1`、19 份当前 fix receipts、当前 recursive/structural/coverage receipts 及相关源码/测试/文档，先用 `git status/check-ignore` 列出精确范围，再重跑 release gate；不放宽 ignore 或 checker。
