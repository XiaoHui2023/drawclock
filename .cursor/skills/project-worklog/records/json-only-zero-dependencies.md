...
- updated: 2026-08-25 15:33 +08:00
...
- 用标准库 `json` 实现唯一输入加载器，并对非 `.json` 后缀、非法 JSON、非对象根节点建立失败契约。
- 删除 Python 运行时依赖、wheelhouse 生成和安装流程，更新包清单、文档和 Release 门。
- 在无第三方 site-packages、无网络环境消费源码与冻结程序。
