...
- **用户列偏好**：新增通用 `layout_column` 字段，相同字符串或整数值形成强列偏好；布局为互不依赖的成员增加最少层宽并共列，祖先与后代同值时仍保持向右连接。字段不依赖 mux 或器件名字，也不写入器件对象属性。
- **列偏好门禁**：新增三条前后深度不同、根集合不同的 mux 支路示例；无提示时占三列，同值后合为一列。Agent schema 10 增加列错位对象报告，并用整列位移、非法字段值和因果冲突播种故障。
