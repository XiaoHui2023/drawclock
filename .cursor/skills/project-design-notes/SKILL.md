...
- 当前维护与发行证明以 16/64/128 个末端时钟的代表图为界；512、1024、2048、4096 高压力输入和专属门禁已移除。CLI 不设置固定节点上限，但超出当前维护范围不得表述为已验证。
...
- 用户问题修复后必须运行有界的五轮递归攻击：精确回归、声明顺序变形、实例重命名、mux 参数组合和跨特性复杂组合。任一目标问题再次出现时连续轮数清零并从第一轮重启；生成/入口/Oracle 运行错误不获得 clean。发行 CI 现场重跑，并校验轮次 exact-set、当前源码及检测器哈希。
