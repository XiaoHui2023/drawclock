...
- `from` 与 `source` 都是零入度器件，不含导出或跨图特殊语义；其中同一输出端口连接多个目标的 `from` 代表单一共享参考网络，必须保持一个物理设施和一条源侧纵向分发总线，不能用显示副本把它拆散。
...
- 公共路径与私人路径进入同一组 mux 前的链深可以不同。典型数组中，唯一公共 `from` 分别连接每路独立 `gate_i→mux_i`，私人 `from_i` 则连接各自 `gate_i→div_i→mux_i`。共享总线身份由公共 `from` 的实际输出端口决定，不要求公共根、私人根或中间器件同列；分层、坐标与布线共同比较后，链深差异不能单独成为碎裂公共主干的理由。
- 多个零入度节点直接进入同一多输入汇聚器件时，以共享目标闭包形成原子 cohort。首列是次级美观偏好：联合回第一列若不增加交叉、跨线、异网重叠、节点/标签碰撞或折点则必须接受，即使直线变长；若任一硬质量退化，则允许相关根靠近下级直连并保留不同列。节点 kind、名称、额外输出和链深不改变该规则。
- 普通零入度 gate/source 的消费簇相距很远且服务代价更低时，可以创建保持逻辑身份不变的显示副本；零入度多扇出 `from`、中间与末端器件不能复制。
...
- 质量检查是独立于设计制作的终态系统。`tools/svg_graph_inspector.py` 从最终 SVG 与输入 JSON 提取全部节点、显示框/中心/行列、每条边的完整点序列/折点/正交线段/方向/交叉伙伴/异网重叠，以及每个网络的设施、物理起点、目标轴和源侧首纵总线；它不导入生产布局模块。
...
- 当前维护与发行证明只覆盖少于 512 节点的公开示例；会产生 520 节点的 128-clock 织网和 512、1024、2048、4096 高压力输入及专属门禁均已移除。CLI 不设置固定节点上限，但 512 及以上不得表述为已验证。
...
- 用户指定公共/私人器件类型、mux 输入数或 mux 后是否直连 clock 时，成功边界必须逐字段匹配这些结构并从最终 SVG 绑定全部对应边；`source→mux3→cell→clock` 不得替代 `from→mux2→clock` 的覆盖声明。
