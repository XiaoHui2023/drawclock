...
@dataclass
class DivNComponent(DivComponent):
    """Programmable divider; center label shows DIV_N."""
    ...
    def _center_labels(self) -> tuple[tuple[float, float, str, int], ...]:
        ...
        return (
            ...
            (cx, cy + DIV_LABEL_Y_OFFSET, "DIV_N", DIV_CENTER_FONT_PX),
        )
...
{body}
  ...
  <text x="{cx}" y="{cy + DIV_LABEL_Y_OFFSET}" font-size="{DIV_CENTER_FONT_PX}" fill="{STROKE}" text-anchor="middle" dominant-baseline="middle">DIV_N</text>
{stubs}
...
"""
    ...
    def verify_geometry(self) -> None:
        ...
        if ">DIV_N</span>" not in html:
            raise ValueError("div_n labels must show DIV_N")
        if f"font-size:{DIV_CENTER_FONT_PX}px" not in html:
            raise ValueError("div_n DIV_N label must use fixed smaller font")
