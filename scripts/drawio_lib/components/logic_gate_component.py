...
from drawio_lib.components import logic_gate_geometry as lgeom
...
class LogicGateComponent:
    ...
    def __post_init__(self) -> None:
        self._g = lgeom.compute_logic_gate_geometry(
            dual_input=self.dual_input,
            with_bubble=self.with_bubble,
            buffer=self.buffer,
        )
        ...
        if full_h != self._g.cell_h:
            self._g = lgeom.reheight_logic_gate_geometry(self._g, full_h)
        pad = sgeom.side_pad_x(sgeom.W)
        port_cells = [(port.anchor.cell_x, port.anchor.cell_y) for port in self._g.inputs]
        port_cells.append((self._g.output.anchor.cell_x, self._g.output.anchor.cell_y))
        self._body_g = sgeom.compute_geometry(
            "both",
            margin_x=4,
            port_cells=tuple(port_cells),
        )
    ...
    @property
    def g(self) -> lgeom.LogicGateGeometry:
        return self._g
    ...
    def label_html(self) -> str:
        ...
        return (
            ...
            f"{stretch_body_layer(body, view_w=self.w, view_h=self.graphic_h, overlays=())}"
    ...
    def cell_style(self) -> str:
        bare = [lgeom.port_drawio_point(port) for port in self._g.inputs]
        bare.append(lgeom.port_drawio_point(self._g.output))
    ...
    def preview_svg(self) -> str:
        ...
        stub_lines = []
        for port, color in (
            *[(port, "#c00") for port in self._g.inputs],
            (self._g.output, "#090"),
        ):
            stub_lines.append(
            ...
            anchor = port.anchor
            stub_lines.append(
                f'  <circle cx="{anchor.cell_x}" cy="{anchor.cell_y}" r="2.5" fill="{color}"/>'
            )
        stubs = "\n".join(stub_lines)
...
"""
    ...
    def verify_geometry(self) -> None:
        ...
        ports = (*self._g.inputs, self._g.output)
        for point, port in zip(points, ports):
            anchor = port.anchor
            if len(point) != 5 or point[2] != float(lgeom.POINT_FIXED):
...
def bind_module(module: object, component: LogicGateComponent) -> None:
    mapping = {
